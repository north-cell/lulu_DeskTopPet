...
- 系统托盘图标：也可以打开显示/隐藏、专注模式、休息、暂停移动、置顶、开机自启动、退出等操作。

### 专注模式 🍊

- 通过噜噜右键菜单或系统托盘菜单进入专注模式。
- 进入后噜噜会回到屏幕右下角，停止移动，并暂时不允许拖拽。
- 噜噜会按专注时长切换专属动画：每 5 分钟切换一次，20 分钟后保持专注陪伴动画。
...
- 🍊 专注模式：右下角陪伴、专属动画、小橘子正计时、学习记录、结束感谢气泡
- 💬 气泡台词
...
- `config/settings.json` 保存窗口大小、置顶、说话间隔、边缘限制、开机自启动和移动速度。
